...
# Delete node pools
gcloud container node-pools delete "${NODE_POOL}" --cluster="${CLUSTER}" --location="${LOCATION}" --project="${PROJECT_ID}" --quiet
gcloud container node-pools delete "${BENCHMARK_NODE_POOL}" --cluster="${CLUSTER}" --location="${LOCATION}" --project="${PROJECT_ID}" --quiet
