...
- An explicitly annotated HPA for each target Deployment. Integration with an
  HPA generated by KEDA requires annotation-propagation support and is not
  covered by the [KEDA + EPP Metrics guide](./README.hpa-epp.md).
...
> [!NOTE]
> This experimental section shows the HPA annotations consumed by the replica
> rebalancer. KEDA owns any HPA it generates, and this guide does not document
> or validate propagation of the required annotation to that HPA. Do not
> independently apply the complete HPA example below to a Deployment already
> managed by a KEDA `ScaledObject`.
...
spec:
  ...
  - type: External
    external:
      metric:
        name: epp_queue_size   # legacy direct-HPA example; KEDA generates its own external metric name
      target:
        type: Value
